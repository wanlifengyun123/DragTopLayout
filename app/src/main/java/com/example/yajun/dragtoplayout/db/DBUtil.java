 ...
 */
package com.example.yajun.dragtoplayout.db;
...
import com.example.yajun.dragtoplayout.db.utils.KeyValue;
import com.example.yajun.dragtoplayout.db.utils.ManyToOne;
import com.example.yajun.dragtoplayout.db.utils.OneToMany;
import com.example.yajun.dragtoplayout.db.utils.TableInfo;
import com.example.yajun.dragtoplayout.db.iml.MessageObservable;
import com.example.yajun.dragtoplayout.db.iml.OnMessageChange;
...
//    }
     ...
     */
//    public static DBUtil create(boolean isDebug) {
...
//    }
    ...
    public static DBUtil create(Context context,DbUpdateListener dbUpdateListener) {
        DaoConfig config = new DaoConfig();
        config.setContext(context);
...
//     *            是否是debug模式（debug模式进行数据库操作的时候将会打印sql语句）
//     */
//    public static DBUtil create(Context context, boolean isDebug) {
//        DaoConfig config = new DaoConfig();
//        config.setContext(context);
...
//     */
//    public static DBUtil create(Context context, String dbName, boolean isDebug) {
//        DaoConfig config = new DaoConfig();
//        config.setContext(context);
//        config.setDbName(dbName);
//        return create(config);
//    }
     ...
     *            数据库名称
     */
    public static DBUtil create(Context context, String targetDirectory,
            String dbName) {
        DaoConfig config = new DaoConfig();
        config.setContext(context);
        config.setDbName(dbName);
...
//                    + "insertContentValues: List<KeyValue> is empty or ContentValues is empty!");
        ...
        if (cursor != null) {
            while (cursor.moveToNext()) {
                ...
                try {
                    ...
                    debugSql(getClass().getName() + e.getMessage());
                }
            }
        }
        if (cursor != null) {
    ...
    private void exeSqlInfo(SqlInfo sqlInfo) {
        if (sqlInfo != null) {
            ...
            debugSql(getClass().getName() + "sava error:sqlInfo is null");
        }
    }
